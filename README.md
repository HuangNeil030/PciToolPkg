...
ShellHexStrToUintn: 將輸入緩衝區的字串轉為整數。

PCI_LIB_ADDRESS: 結合 Bus/Dev/Func 與選定的 Offset 計算目標位址。

gPciIo->Pci.Write: 呼叫 UEFI Protocol 執行實際寫入。

寫入後自動切換回 STATE_VIEW_DUMP，因為畫面重繪時會重新讀取硬體，使用者能立刻看到結果。
...
📝 學習重點摘要
Protocol 使用: 熟練 EFI_PCI_ROOT_BRIDGE_IO_PROTOCOL 的 Pci.Read 和 Pci.Write。

PCI 位址結構: 理解 Bus, Device, Function, Offset 如何組合成 UEFI 的 64-bit Address。

Config Space 結構: 熟悉 Offset 0x00 (ID), 0x08 (Class/Rev), 0x0E (Header Type) 的定義。

UEFI UI 開發: 學習如何使用 SetAttribute 做顏色高亮，以及如何處理鍵盤事件 (ReadKeyStroke)。
